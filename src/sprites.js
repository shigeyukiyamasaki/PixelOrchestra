...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    // ベル部：奥行き 16 で、チューニング管の U ターンは絵の z=13-14（取り付け後は奏者の左 7.2px = 頭のすぐ横）、ベルは z=0-11（中心 2.5px 左）。
    // ベル管は上から見て斜め（頭の横の U ターン → ベルへ寄る）。U ターンと本体の縦管は rows 4-5 の横管でつなぐ
    const bell = makePart(34, 12, 0, 13, (d) => {
      d.r(0, 0, 10, 2, C.gold); d.r(0, 0, 2, 6, C.gold); d.r(8, 0, 2, 6, C.gold);             // チューニング管（U ターン）
      d.r(0, 4, 19, 2, C.gold);                                                               // 横管（x 0-1）＋ベル管
      d.r(18, 3, 3, 4, C.gold); d.r(21, 2, 3, 6, C.gold); d.r(24, 1, 3, 8, C.gold); d.r(27, 0, 3, 12, C.gold); d.r(30, 0, 2, 12, C.gold2); // ベル
      d.r(10, 4, 8, 1, '#f3d27a');
    }, { res: 2, depth: 16, z0: -6,
         side: (d) => { d.disc(6, 6, 6, F); d.r(13, 0, 3, 6, F); d.r(0, 4, 16, 2, F); },
         top: (d) => {
           d.r(0, 13, 10, 2, F); d.r(0, 4, 2, 11, F);                                          // U ターン（z 13-14）・横管（x 0-1, z 4-14）
           for (let x = 8; x < 18; x++) d.r(x, Math.round(13 - (x - 8) * 0.8), 1, 2, F);      // ベル管（斜め：z 13 → 5）
           d.r(18, 4, 3, 4, F); d.r(21, 3, 3, 6, F); d.r(24, 1, 3, 10, F); d.r(27, 0, 5, 12, F); // ベル
         },
         carve: (x, y, z) => {
           if (x < 2) return !((z >= 13 && z <= 14) || (y >= 4 && y <= 5));                    // x 0-1：U ターンの脚（z 13-14）と横管（rows 4-5）だけ残す
           if (x < 20) return false;
           const R = x >= 27 ? 6 : x >= 24 ? 4 : x >= 21 ? 3 : 2; const r = R - 1.2; const dy = y - 5.5, dz = z - 5.5; return dy * dy + dz * dz < r * r; // ベルの穴
         } });
