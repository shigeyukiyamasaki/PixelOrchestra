...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    const bell = makePart(48 - BB, 20, 14, 13, (d) => {
      ...
      // ベル（rows 6-17）。管（半径 1）から 3px 上がって半径 4、次に 2px 上がって半径 6（2026-09-12 ユーザー指定）。
      // 半径 6 は z 方向の余白（depth 24・中心 z17.5）の限界なので、これ以上は広げられない。
      // 一番広がった部分は濃い色のフチだけ 2px（明るい色の帯は無し）
      d.r(32 - BB, 8, 2, 8, C.gold); d.r(34 - BB, 6, 2, 12, C.gold2);
      d.r(16, 11, 16 - BB, 1, '#f3d27a');
    }, { res: 2, depth: 24, z0: -18,
         side: (d) => { d.disc(18, 11, 6, F); d.r(17, 11, 2, 7, F); },
         top: (d) => {
           d.r(0, 17, 33 - BB, 2, F);                                                          // ベル管（z 17-18）
           d.r(32 - BB, 14, 2, 8, F); d.r(34 - BB, 12, 2, 12, F); // ベル（中心 z 17.5）：半径 4→6
         },
         carve: (x, y, z) => {
           if (x < 34 - BB) return false;
           const R = 6; const r = R - 1.2; const dy = y - 11.5, dz = z - 17.5; return dy * dy + dz * dz < r * r; // ベルの穴（フチの内側だけ抜く）
         } });
