...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    // ベル部：チューニング管の U ターン → ベル管 → ベル が一直線（絵の z=17-18、取り付け後は奏者の左 7.5px = 頭のすぐ横）。
    // 2026-09-10 ユーザー指摘：ベルは U ターンの延長線上。マウスピース側の横管（rows 4-5, x 0-1）で本体の縦管（左右中央）とつなぐ
    const bell = makePart(34, 12, 0, 13, (d) => {
      ...
      d.r(10, 4, 8, 1, '#f3d27a');
         ...
         top: (d) => {
           d.r(0, 17, 20, 2, F); d.r(0, 5, 2, 14, F);                                          // U ターン＋ベル管（z 17-18）・横管（x 0-1, z 5-18）
           d.r(18, 16, 3, 4, F); d.r(21, 15, 3, 6, F); d.r(24, 13, 3, 10, F); d.r(27, 12, 5, 12, F); // ベル（中心 z 17.5）
         },
         carve: (x, y, z) => {
           if (x < 2) return !((z >= 17 && z <= 18) || (y >= 4 && y <= 5));                    // x 0-1：U ターンの脚（z 17-18）と横管（rows 4-5）だけ残す
