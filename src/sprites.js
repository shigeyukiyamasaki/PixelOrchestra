...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    const bell = makePart(48 - BB, 20, 14, 13, (d) => {
      ...
      // ベル（rows 6-17）。管（半径 1）から最初だけ 2px 上がって半径 3、あとは 1px ずつ 4→5→6（2026-09-12 ユーザー指定）。
      // 一番広がった部分は濃い色のフチだけ 2px（明るい色の帯は無し）
      d.r(32 - BB, 9, 2, 6, C.gold); d.r(34 - BB, 8, 2, 8, C.gold); d.r(36 - BB, 7, 2, 10, C.gold); d.r(38 - BB, 6, 2, 12, C.gold2);
      d.r(16, 11, 16 - BB, 1, '#f3d27a');
    }, { res: 2, depth: 24, z0: -18,
         side: (d) => { d.disc(18, 11, 6, F); d.r(17, 11, 2, 7, F); },
         top: (d) => {
           d.r(0, 17, 33 - BB, 2, F);                                                          // ベル管（z 17-18）
           d.r(32 - BB, 15, 2, 6, F); d.r(34 - BB, 14, 2, 8, F); d.r(36 - BB, 13, 2, 10, F); d.r(38 - BB, 12, 2, 12, F); // ベル（中心 z 17.5）：半径 3→4→5→6
         },
         carve: (x, y, z) => {
           if (x < 34 - BB) return false;
           const R = x >= 38 - BB ? 6 : x >= 36 - BB ? 5 : 4; const r = R - 1.2; const dy = y - 11.5, dz = z - 17.5; return dy * dy + dz * dz < r * r; // ベルの穴
         } });
