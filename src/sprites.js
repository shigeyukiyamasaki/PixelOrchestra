...
export const INSTRUMENT = {
  ...
  hihat: () => makePart(32, 36, 16, 0, (d) => {
    d.r(0, 0, 32, 2, C.gold2); d.r(0, 2, 32, 1, C.gold); d.r(0, 3, 32, 1, C.gold2);          // 上のシンバル：カップ・表・縁
    d.r(0, 4, 32, 2, C.silver);                                                              // 2 枚の間のロッド
    d.r(0, 6, 32, 1, C.gold2); d.r(0, 7, 32, 1, C.gold); d.r(0, 8, 32, 1, C.gold2);          // 下のシンバル：縁・裏・カップ（下向き）
    d.r(0, 9, 32, 27, C.silver2);                                                            // 支柱・脚
  }, { res: 2, depth: 32, z0: 0, top: (d) => { d.disc(16, 16, 15, F); },
       carve: (x, y, z) => {
         const dx = x - 15.5, dz = z - 15.5, r = Math.hypot(dx, dz);
         if (y <= 1) return r > (y === 0 ? 2.5 : 4.5);                                      // カップ
         if (y <= 3) return r > (y === 2 ? 15.5 : 14.5);                                     // 上のシンバル（縁は 1 セル内側）
         if (y <= 5) return r > 1;                                                           // ロッド
         if (y <= 7) return r > (y === 6 ? 14.5 : 15.5);                                     // 下のシンバル
         if (y === 8) return r > 4.5;                                                        // 下のカップ
