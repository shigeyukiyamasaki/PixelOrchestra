...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    // ベル部：1 本の管の経路として描く。本体の縦管（下の内管の端から上へ）→ 横管（rows 4-5, x 2-3）で左へ 7.5px →
    // U ターンの後ろ脚を上る → 天井 → 前脚を下りる → ベル管 → ベル。U ターン・ベル管・ベルは一直線（絵の z=17-18 = 頭の左横）
    // （2026-09-10 ユーザー指摘：ベルは U ターンの延長線上、かつ 1 本の管でつながって見えること）
    const bell = makePart(34, 12, 0, 13, (d) => {
      d.r(2, 0, 10, 2, C.gold); d.r(2, 0, 2, 6, C.gold); d.r(10, 0, 2, 6, C.gold);            // チューニング管の U ターン（後ろ脚 x 2-3・天井・前脚 x 10-11）
      d.r(2, 4, 2, 2, C.gold);                                                                // 横管の端（x 2-3, rows 4-5。奥行きは top で与える）
      d.r(10, 4, 9, 2, C.gold);                                                               // ベル管（前脚の根元 → ベル）
      ...
      d.r(12, 4, 6, 1, '#f3d27a');
         ...
         side: (d) => { d.disc(18, 6, 6, F); d.r(17, 0, 2, 6, F); d.r(0, 4, 24, 2, F); },
         top: (d) => {
           d.r(2, 17, 17, 2, F); d.r(2, 5, 2, 14, F);                                          // U ターン＋ベル管（z 17-18）・横管（x 2-3, z 5-18）
           d.r(18, 16, 3, 4, F); d.r(21, 15, 3, 6, F); d.r(24, 13, 3, 10, F); d.r(27, 12, 5, 12, F); // ベル（中心 z 17.5）
         },
         carve: (x, y, z) => {
           if (x < 4) return !((z >= 17 && z <= 18) || (y >= 4 && y <= 5));                    // x 2-3：後ろ脚（z 17-18）と横管（rows 4-5）だけ残す
           if (x < 20) return false;
           const R = x >= 27 ? 6 : x >= 24 ? 4 : x >= 21 ? 3 : 2; const r = R - 1.2; const dy = y - 5.5, dz = z - 17.5; return dy * dy + dz * dz < r * r; // ベルの穴
         } });
