...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    const body = makePart(34, 20, 0, 13, (d) => {
      ...
      d.r(3, 13, 30, 1, C.gold2); d.r(3, 19, 30, 1, C.gold2);                                 // 内管（上・下。外管が伸びると露出）。間隔 6 行・長さ 30（2026-09-12 ユーザー指定で 1.5 倍）
    ...
    const bell = makePart(48 - BB, 20, 14, 13, (d) => {
      d.r(0, 11, 33 - BB, 1, C.gold);                                                         // ベル管（前へ）。半径 0.5px＝1 行（2026-09-12 ユーザー指定）
      // ベル（rows 6-17）。半径は 1→2→3→4→5→6 と 1px ずつ。段の「幅」（管の軸方向）は管に近い順に 5, 4, 3, 2, 1, 1 px（2026-09-12 ユーザー指定）。
      // 根元をゆっくり・先端を急に開く形。一番広がった部分は濃い色のフチ（明るい色の帯は無し）
      d.r(32 - BB, 11, 5, 2, C.gold); d.r(37 - BB, 10, 4, 4, C.gold); d.r(41 - BB, 9, 3, 6, C.gold); d.r(44 - BB, 8, 2, 8, C.gold); d.r(46 - BB, 7, 1, 10, C.gold); d.r(47 - BB, 6, 1, 12, C.gold2);
    }, { res: 2, depth: 24, z0: -18,
         side: (d) => { d.disc(18, 11, 6, F); d.r(17, 11, 2, 7, F); },
         top: (d) => {
           d.r(0, 17, 33 - BB, 1, F);                                                          // ベル管（z 17。半径 0.5px）
           d.r(32 - BB, 17, 5, 2, F); d.r(37 - BB, 16, 4, 4, F); d.r(41 - BB, 15, 3, 6, F); d.r(44 - BB, 14, 2, 8, F); d.r(46 - BB, 13, 1, 10, F); d.r(47 - BB, 12, 1, 12, F); // ベル（中心 z 17.5）：半径 1→2→3→4→5→6
         },
         carve: (x, y, z) => {
           if (x < 41 - BB) return false;                                                     // 根元の 2 段（半径 1・2）は塞いだまま：管からの立ち上がり
           const R = x >= 47 - BB ? 6 : x >= 46 - BB ? 5 : x >= 44 - BB ? 4 : 3; const r = R - 1.2; const dy = y - 11.5, dz = z - 17.5; return dy * dy + dz * dz < r * r; // ベルの穴
         } });
    // ベル管の左右位置。スライドの管（楽器ローカル z ≒ 3px）とぶつからないよう外側へ逃がす。
    // 左手はスライドを握るので、ベル管に合わせる必要はない（2026-09-12 ユーザー指定で 6*VOX → 10*VOX）
    ...
    const slide = makePart(33, 7, 0, 0, (d) => {
      d.r(0, 0, 30, 1, C.gold); d.r(0, 6, 30, 1, C.gold); d.r(28, 0, 3, 7, C.gold); d.r(30, 1, 1, 5, C.gold2); // 管・U 字
      d.r(1, 0, 1, 7, C.silver);                                                                // 支柱
      // 奥行きは 1（管の太さと同じ）。スライド部は -60° ロールしているので、奥行きが大きいと
      // 画面上では管が縦長のリボンに見える（2026-09-12 ユーザー指定で 3 → 1）
    }, { res: 2, depth: 1, z0: -0.5 });
