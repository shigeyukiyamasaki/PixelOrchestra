...
export const INSTRUMENT = {
  ...
  trombone: () => {
    ...
    const bell = makePart(48 - BB, 20, 14, 13, (d) => {
      d.r(0, 11, 33 - BB, 2, C.gold);                                                         // ベル管（前へ）
      // ベル（rows 6-17）。半径は 2→3→4→5→6 と 1px ずつ。段の「幅」（管の軸方向）は管に近い段だけ 3px、あとは 2px（2026-09-12 ユーザー指定）。
      // 一番広がった部分は濃い色のフチだけ 2px（明るい色の帯は無し）
      d.r(32 - BB, 10, 3, 4, C.gold); d.r(35 - BB, 9, 2, 6, C.gold); d.r(37 - BB, 8, 2, 8, C.gold); d.r(39 - BB, 7, 2, 10, C.gold); d.r(41 - BB, 6, 2, 12, C.gold2);
      d.r(16, 11, 16 - BB, 1, '#f3d27a');
    }, { res: 2, depth: 24, z0: -18,
         side: (d) => { d.disc(18, 11, 6, F); d.r(17, 11, 2, 7, F); },
         top: (d) => {
           d.r(0, 17, 33 - BB, 2, F);                                                          // ベル管（z 17-18）
           d.r(32 - BB, 16, 3, 4, F); d.r(35 - BB, 15, 2, 6, F); d.r(37 - BB, 14, 2, 8, F); d.r(39 - BB, 13, 2, 10, F); d.r(41 - BB, 12, 2, 12, F); // ベル（中心 z 17.5）：半径 2→3→4→5→6
         },
         carve: (x, y, z) => {
           if (x < 35 - BB) return false;                                                     // 根元の段（半径 2）は塞いだまま：管からの立ち上がり
           const R = x >= 41 - BB ? 6 : x >= 39 - BB ? 5 : x >= 37 - BB ? 4 : 3; const r = R - 1.2; const dy = y - 11.5, dz = z - 17.5; return dy * dy + dz * dz < r * r; // ベルの穴
         } });
