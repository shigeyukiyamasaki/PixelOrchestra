...
function settings() {
  ...
  return {
    ...
    metalSpec: num('metalSpec', 2), metalThr: num('metalThr', 0.35),   // 金属の楽器のツヤ／金属だけのブルーム閾値（2026-09-23 ユーザー指定。ハイライトの鋭さは楽器ごとの固定値にしてスライダーは廃止）
...
let lastMetalSpec = null, lastMetalThr = null;   // 金属のツヤ・金属だけのブルーム閾値（変わった時だけシーンを走査する）
...
function animate() {
  ...
  if (engine && conductor && roll) {
    ...
    if (s.metalSpec !== lastMetalSpec || s.metalThr !== lastMetalThr) { lastMetalSpec = s.metalSpec; lastMetalThr = s.metalThr; applyMetalLook(scene, s.metalSpec, s.metalThr); }
