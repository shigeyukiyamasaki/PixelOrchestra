...
const VARIANT = {
  ...
  contrabass: { spine: true, gaze: true, floorStand: true, inst: { pos: [3, 0, 4], rot: 0 }, held: { R: 'bow' }, pole: { R: [0.6, -0.6, 0.5] }, /* 右肘は外・下・前（真下だと肘が体の後ろ z -0.6 に落ちる。2026-09-16 ユーザー指定）。[0.3,-0.4,1] のように前へ出しすぎると IK が 2 解を行き来して腕が毎フレーム 2〜9px 跳ねる（チラつき）。この向きなら 0.2px */ bow: { contact: [0.5, 16], world: 2.95, sMin: 5, sMax: 15 }, leftHand: [0, 32], // 弓の接点は駒（y 15）のすぐ上（pivot 基準）。指板（〜y 16.5）の上で弾くと、弓を持つ手が指板に重なった。手は弦から 5px 以上離す（2026-09-16 ユーザー指摘） vib: [0, 1, 0], // 左手はネック（胴の上端 row 16 = y 30 より上）を持つ。胴を腕が貫通していたため（2026-09-16 ユーザー指摘）
