...
const CELLO_Q = new THREE.Quaternion().setFromEuler(new THREE.Euler(-0.4, 0, 0.2)); // 約 23° 後ろへ傾けて上部を胸に寄りかからせ、11° 左へ倒してネックを頭の横に出す（1.4 倍にしたので 29° → 23°。2026-09-11）
...
const VARIANT = {
  ...
  flute:      { inst: { pos: [-1, 31.5, 4], rot: -0.15 }, hands: { L: [6, 0.3], R: [13, 1.3] }, kind: 'flute', handDirs: { L: [0, 1, 0], R: [0, 0.5, 1] }, gazeDown: 0.05, pole: { R: [1, -0.7, -0.2] }, // 右肘は真横でなく斜め下へ（2026-09-11）
                p3: { pos: [-1, 31.5, 5], rot3: [0, -0.4, -0.35], hands: { L: [6, 0.3, 0.6], R: [13, 1.3, 0.8] } } }, // 右手は管より少し上。管を右下がり 20° にして右肘が斜め下に来るように（2026-09-11）
  // ピッコロ：フルートと同じ構え。管が短いので手の間隔は狭い
  piccolo:    { inst: { pos: [-1, 31.5, 4], rot: -0.15 }, hands: { L: [4, 0.3], R: [8, 1.3] }, kind: 'flute', handDirs: { L: [0, 1, 0], R: [0, 0.5, 1] }, gazeDown: 0.05, pole: { R: [1, -0.7, -0.2] },
                p3: { pos: [-1, 31.5, 5], rot3: [0, -0.4, -0.35], hands: { L: [4, 0.3, 0.6], R: [8, 1.3, 0.8] } } },
