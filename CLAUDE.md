...
- **パペット × ドット絵 × 3D ステージ**。絵は「ボクセル（ドット絵の押し出し）」固定（2026-09-10 ユーザー確定。2D の板は UI から撤去、実装は sprites.js に残置）。体の向きは指揮者固定。ユーザーがアングルを回せることが要件
  - **例外：銅鑼とチューブラーベル**は体を楽器へ向け、首だけで指揮者を見る（銅鑼は `bodyYaw`、チューブラーベルは `turn` で 60°。2026-09-25 ユーザー承認）。
    チューブラーベルは両手のハンマーで 2 列の管を叩くため、指揮者向きのままだと左手が低音の管に届かなかった
...
- **ハープとピアノの形は `assets/voxel/harp.json` / `piano.json` が優先される**（2026-09-24〜25、ユーザーが編集画面 `/edit.html?part=harp|piano` で作った）。
  JSON がある間は `sprites.js` の手続き的な形（`harpCell` の `HARP_NECK_KEYS` 等、`pianoEdge`）を変えても画面は変わらない。
  形を直す時は JSON を直すか、編集画面で直してもらう。JSON を消すと手続き的な形に戻る
- **編集画面ではピアノとチェレスタが左右反転して見える**。奏者の体が鏡像なので、データの時点で左右を逆にしてある（`pianoCell` / `celestaCell` の中で x を反転）。
  編集画面で右に見えるまっすぐな側板が、舞台では奏者の左（低音側）。ハープは反転していない
- **手の位置は楽器の形を前提にしている**：ハープの弦の奥行き（`puppet.js` の `HARP_STRING_Z`）・共鳴胴の上端（`HARP_LEAN_CELLS`）、
  ピアノ／チェレスタの鍵盤の高さと手前の縁。編集でこれらを大きく動かすと手とずれるので、動かしたら手の設定も合わせる
- **ピアノの補完**：ユーザーが胴の一番下の行（y 33）の輪郭だけ直した時は `node tools/piano_fill.js` で胴（y 20〜32）と蓋（y 12〜19）を
  その輪郭に合わせる（書き換え前に `assets/voxel/backup/` へ自動で退避）。実行後、編集画面のタブは再読み込みしてもらう（そのまま保存すると補完が消える）。
  **y 12〜32 を作り直すので、補完のあとにユーザーが手で直した所（突っかい棒・縁など）も消える**。
  頼まれた時だけ実行し、先に `--dry-run` で変わるセル数を見る（2026-09-25 時点の piano.json は補完後に手直し済みで、かけると 68 セル戻る）
