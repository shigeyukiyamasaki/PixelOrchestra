...
- ドット絵は最近傍補間（`TOOL_CRAFT_MEDIA.md` §1-3）で、ドット絵自体にはブルームを掛けない（2026-09-09 の設計時に Claude が置いた方針。ユーザー指示ではない）。ブルームを掛けるなら §2-4 のとおり対象を絞る（2026-09-16：太陽だけの選択的ブルームを採用。本編は等倍転写でぼかさない）
